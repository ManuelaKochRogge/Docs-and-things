...
# 3.2. Gründe für Abbruch aggregieren
...
# 4.2. Korrelation mittels Funktion "cor2" für mixed data ermitteln
## Quelle Funktion cor2:
# author  : Srikanth KS (talegari)
# license : GNU AGPLv3 (http://choosealicense.com/licenses/agpl-3.0/)

cor_num_clust<-cor2(clust_Abbrecher)
View(cor_num_clust) # Identifikation: imma_jahr, exma_jahr, geburtsjahr, hzb_jahr,
# zzgl. auch Matrikelnummer, staat, dienst entfernen
...
# 5.2 auffällige Variablen genauer betrachten
...
head(clust_A)
summary(clust_A)
lapply(clust_A[  ,6:11], sd)

#############################################
# 6. auf fehlende Werte prüfen
...
## geeigneten CLustering-Algorithmus ermitteln
# Vektor mit Algorithmen

m <- c( "average", "single", "complete", "ward")
names(m) <- c( "average", "single", "complete", "ward")

# Funktion um Agglomerativ-Koeffizienten zu ermitteln
ac <- function(x) {
...
write.xlsx(k3_abbrecher, file="k3_abbrecher.xlsx")

#############################################
# 11. Visualisierung k = 3
ggplot(k3_abbrecher, aes(x = alter_bei_hzb, y = erreichte_cp, color = factor(cluster)))+ geom_point()
ggplot(k3_abbrecher, aes(x = hzb_note, y = erreichte_cp, color = factor(cluster)))+ 
  geom_point()+scale_x_log10()+scale_y_sqrt(breaks = 150)
